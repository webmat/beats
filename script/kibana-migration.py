...
def migration():
    ...
    print "Postfix all ids with -7x"
    ids = get_replacable_ids()
    rename_entries(ids)

    print "Postfix all titles with ` 7.x`"
        ...
        with open(file, 'r') as f:
            ...
            for v in objects["objects"]:
                ...
                if "title" in v["attributes"]:
                    if "7.x" not in v["attributes"]["title"]:
                        titles['"' + v["attributes"]["title"] + '"'] = '"' + v["attributes"]["title"] + " 7.x" + '"'

                if "visState" in v["attributes"] and "title" in v["attributes"]["visState"]:
                    if "7.x" not in v["attributes"]["visState"]["title"]:
                        titles['"' + v["attributes"]["visState"]["title"] + '"'] = '"' + \
                            v["attributes"]["visState"]["title"] + " 7.x" + '"'
...
def get_files():
    all_beats = '../*/_meta/kibana/6/dashboard/*.json'
    module_beats = '../*/module/*/_meta/kibana/6/dashboard/*.json'
    heartbeat = '../hearbeat/monitors/active/*/_meta/kibana/6/dashboard/*.json'
    xpack_module_beats = '..x-pack/*/module/*/_meta/kibana/6/dashboard/*.json'
